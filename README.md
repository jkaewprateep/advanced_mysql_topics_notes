...
🐑💬 ➰ In basic report tools, they separate ```details area```, ```aggregation area```, and ```asides area``` and this concept helps with data population and dataset organization. </br>

🐣💬 ```SAP!!!``` ```ERP!!!``` 🧸💬 ```Template report``` and ```business report``` they are commercials name. 👤💬 ```Jasper``` 👤💬 ```Avaya Cognos``` 👤💬‼️ ```IBM``` 👤💬 ```Google Visulaization``` ... </br>
...
🐑💬 ➰ The ```statistics server manages of statistics data in memory in the same way as the data tables does```, this is for information debugging and communication with the same expectation. All methods are build permanent and encapsulated solution for the project achievements. </br>
...
👧💬 🎈 Sample of search for ```full name database column field with index usages```, ```possible_key = IdxReverseFullName``` . </br>
...
🐐💬 For networks tracing simple scripts monitor for ```select``` or ```update``` because exporting data required joined of the procedure from the ```blank select``` or ```blank select procedure``` . </br>
